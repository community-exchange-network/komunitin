...
export const useMergedResources = (
  ...
  const loadNext = async () => {
    // Call loadNext for each type that has hasNext true. We could improve this by
    // avoid calling loadNext for types where we still have sufficient loaded items to
    // continue merging. However this can't be easily determined using typeResources lengths
    // because these resource arrays may be filled with cached results.
    const promises = []
    for (let i = 0; i < types.length; i++) {
      if (typeHasNexts[i].value) {
