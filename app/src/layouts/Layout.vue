    ...
    >
      ...
      >
        ...
        <q-footer class="lt-md" v-if="myMember && isRootPage">
          <q-toolbar class="bg-surface text-primary">
            <q-tabs 
              breakpoint="1024" 
              class="full-width"
              switch-indicator
              ...
              align="justify"
            >
              <q-route-tab class="nav-tab" :to="{ name: 'Home' }" name="home" icon="home" :label="t('home')" />
              <q-route-tab class="nav-tab" :to="{ name: 'TransactionList', params: { code: groupCode, memberCode: myMember.attributes.code}}" name="account" icon="swap_horiz" :label="t('transactions')" />
              <q-route-tab class="nav-tab" :to="{ name: 'Group', params: { code: groupCode }}" name="group" icon="people" :label="t('group')" />
            </q-tabs>
          </q-toolbar>
        </q-footer>
      </q-layout>
