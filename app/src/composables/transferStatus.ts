...
const STATUS_MAP = {
  new:       { color: 'info',        label: (t: T) => t('new') },
  pending:   { color: 'warning',     label: (t: T) => t('pending') },
  accepted:  { color: 'teal',        label: (t: T) => t('accepted') },
  committed: { color: 'positive',    label: (t: T) => t('committed') },
  rejected:  { color: 'negative',    label: (t: T) => t('rejected') },
